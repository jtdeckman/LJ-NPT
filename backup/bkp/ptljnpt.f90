...
SUBROUTINE ATTSWAP(Q,TFLAG)
  ...
  INTEGER, SAVE :: REQ(100),REQT(100),REQV(100),ierr
  ...
  IF(NVOUT.GT.0) THEN
     ...
     ELSE
        IF(VSENDNR) THEN
           CALL MPI_IRECV(2,1,MPI_INTEGER,0,2,MPI_COMM_WORLD,REQ(2),ierr)      
           VSENDNR=.FALSE.
        ENDIF
  ...
  ELSE              
     ...
     IF((CSTEP(1).GE.SWPINT(1)).AND.(.NOT.TFLAG)) THEN        
        ...
        IF(SENDNR) THEN                                                                ! Send new receive
           CALL MPI_IRECV(3,1,MPI_INTEGER,mynode-1,3,MPI_COMM_WORLD,REQ(3),ierr)
           SENDNR=.FALSE.
        ENDIF
