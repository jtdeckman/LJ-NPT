     ...
     DOUBLE PRECISION :: VLEN,PMIN,PMAX,RFRSH,MAXBL,BLDIFF, PWCUT
     ...
     INTEGER :: NEQL,FRCFRQ,PWFRQ,EVFREQ
     CHARACTER*2 :: TAG
     LOGICAL :: INSIDE
     ...
     OPEN(UNIT=8,FILE='inputf.dat',STATUS='OLD')        
        ...
        READ(8,*) PWCUT                              ! Cut-off for computing pair-wise forces
        READ(8,*) EVFREQ                             ! Frequency for computing / writing eigenvalues
     CLOSE(8)   
     ...
     IF(.NOT.INSIDE) THEN
        ...
        GOTO 70
     ELSE
        CALL INITIALIZE_LJNPTMOD(NATOMS,TMAX,TMIN,PMIN,PMAX,BLX,NP,GRIDSIZE,mynode,ENOT,EG,BLDIFF,EVFREQ)
        CALL INITIALIZE_LJNPTMC(NSTEP,UF,WRF,WF,SWPINT,SL,VLEN,RCON,Q,CONT,NATOMS,VSFREQ,RFRSH,MAXBL,VOF,NEQL,FRCFRQ,PWFRQ,PWCUT)        
