SUBROUTINE UPDATE(Q)
  ...
  IF((NTOTAL.GT.NEQUIL).AND.(PWFREQ.GT.0).AND.(MOD(NTOTAL,PWFREQ).EQ.0)) CALL PWFRCOUT(Q)
  IF((NTOTAL.GT.NEQUIL).AND.(EVFREQ.GT.0).AND.(MOD(NTOTAL,EVFREQ).EQ.0)) CALL HESSIAN(Q)
...
SUBROUTINE FRCOUT(Q)
  ...
  DOUBLE PRECISION :: Q(3,N_ATOM), FRC(3,N_ATOM), FRCR(3,N_ATOM),FRCA(3,N_ATOM)
  DOUBLE PRECISION :: MAGF, MAGR, MAGA, DMAGF

  CALL CALCFORCES(Q,FRC,FRCR,FRCA)
  ...
  DO I=1,N_ATOM
    ...
    DO J=1,3
      MAGF = MAGF + FRC(J,I)**2
      MAGR = MAGR + FRCR(J,I)**2
      MAGA = MAGA + FRCA(J,I)**2
      DMAGF = DMAGF + (FRCA(J,I) + FRCR(J,I))**2
    ENDDO

    WRITE(FRCFIND,"(4F12.5)") MAGF**0.5D0, DMAGF**0.5D0, MAGR**0.5D0, MAGA**0.5D0
...
SUBROUTINE PWFRCOUT(Q)
  USE LJNPTMOD
  USE LJNPTMC
  IMPLICIT NONE
  INTEGER :: I, NPAIRS
 
  DOUBLE PRECISION :: Q(3,N_ATOM), FRC(3,(N_ATOM*N_ATOM)/2)

  CALL CALCFORCESPW(Q,FRC,NPAIRS,PWCUT)

  DO I=1,NPAIRS
     WRITE(PWFRCFIND,"(3F12.5)") FRC(1,I), FRC(2,I), FRC(3,I)
  ENDDO

  CALL FLUSH(PWFRCFIND)

END SUBROUTINE PWFRCOUT
